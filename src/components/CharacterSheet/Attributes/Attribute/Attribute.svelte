...
<div class="row">
    <div class="title-column stress-track-title">
        <input style="clear-after: both" maxlength="2" type="number" class="attr-value" min="0" max="10" bind:value={value}/>
    ...
    {#each Array(6) as _, i}
        <div class="column">
            <CheckedItem css="{i<count ? css: grayCss}" number="1" checked="{i < stressTaken}" on:checkedChanged={handleCheckedChanged}/>
        </div>
    {/each}
        </div>
</div>
